...
class DiceBox {
	...
	async loadSounds(){
		...
		if(!this.sounds_table.hasOwnProperty(this.surface)){
			...
			for (let s=1; s <= numsounds; ++s) {
				const clip = await this.loadAudio(this.assetPath + 'dice-assets/sounds/surfaces/surface_'+this.surface+s+'.mp3')
		...
		if(!this.sounds_dice.hasOwnProperty('coin')){
			...
			for (let s=1; s <= numsounds; ++s) {
				const clip = await this.loadAudio(this.assetPath + 'dice-assets/sounds/dicehit/dicehit_coin'+s+'.mp3')
		...
		if(!this.sounds_dice.hasOwnProperty(this.sound_dieMaterial)){
			...
			for (let s=1; s <= numsounds; ++s) {
				const clip = await this.loadAudio(this.assetPath + 'dice-assets/sounds/dicehit/dicehit_'+this.sound_dieMaterial+s+'.mp3')
	...
	setDimensions(dimensions) {
		...
		// Lumière ambiante forte pour un éclairage uniforme
		const ambientLight = new THREE.AmbientLight(0xffffff, this.light_intensity * 1.2);
		this.scene.add(ambientLight);
		
		// Lumière hémisphérique pour un éclairage naturel
		this.light_amb = new THREE.HemisphereLight(0xffffff, 0xffffff, this.light_intensity);
		this.scene.add(this.light_amb);
		
		// Spot principal (haut-gauche)
		this.light = new THREE.SpotLight(this.color_spotlight, this.light_intensity * 0.8);
		...
		// Lumière directionnelle de face (pour voir les faces des dés)
		const frontLight = new THREE.DirectionalLight(0xffffff, this.light_intensity * 0.6);
		frontLight.position.set(0, 0, maxwidth * 3);
		this.scene.add(frontLight);
		
		// Lumière directionnelle du haut
		const topLight = new THREE.DirectionalLight(0xffffff, this.light_intensity * 0.5);
		topLight.position.set(0, maxwidth * 2, maxwidth);
		this.scene.add(topLight);
		
		// Lumière de remplissage (droite)
		const fillLight = new THREE.DirectionalLight(0xffffff, this.light_intensity * 0.3);
		fillLight.position.set(maxwidth, 0, maxwidth);
